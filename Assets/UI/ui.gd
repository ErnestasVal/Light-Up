...
func _do_change_level(go_to_main_menu: bool):
	if go_to_main_menu:
...
func level_change(go_to_main_menu: bool = false):
	Input.action_press(mouse_mode_action)
	Input.action_release(mouse_mode_action)
	_level_change_tween = create_tween()
	_level_change_tween.tween_interval(0.5)
	_level_change_tween.tween_callback(_do_change_level.bind(go_to_main_menu))
	exitLevel()
