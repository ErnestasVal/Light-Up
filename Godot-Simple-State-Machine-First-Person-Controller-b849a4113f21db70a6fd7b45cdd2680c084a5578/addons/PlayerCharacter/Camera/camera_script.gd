...
@export_range(0.0, 0.5, 0.001) var x_axis_sensitivity : float = 0.05
@export_range(0.0, 0.5, 0.001) var y_axis_sensitivity : float = 0.05
...
func _unhandled_input(event) -> void:
	...
	if event is InputEventMouseMotion and !mouse_free:
		rotate_y(-event.relative.x * (x_axis_sensitivity / 10))
		camera.rotate_x(-event.relative.y * (y_axis_sensitivity / 10))
...
func change_fov() -> void:
	...
	if !zoom_on and !zoom_has_occured:
		if state != null and state != "Jump" and state != "Inair":
			...
			fov_tween.tween_property(camera, "fov", cam_fov_per_state[state][0], cam_fov_per_state[state][1])
